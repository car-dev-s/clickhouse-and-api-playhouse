...
Because mutation inputs bind to a raw `Map` rather than a `@Valid`-annotated DTO, the Bean Validation constraints REST enforces (`@NotBlank`, `@NotNull` on `CreateEventRequest` etc.) never run on this path — `createEvent(input: {eventType: "", userId: ""})` succeeds over GraphQL where `POST /api/events` with the same blank fields would be rejected with a 400 from REST's `@Valid`. This mirrors an existing gap in the gRPC transport (`EventGrpcService` has the same unvalidated-input behavior), so it isn't a new divergence introduced by GraphQL, but it's worth naming explicitly here since it's easy to assume schema types imply validation.
